...
const chatListModel = {
    getChatListByUserId: (req) => {
        return new Promise((resolve, reject) => {
            db.query(`SELECT single_chat.last_time, contacts_list.contact_name, contacts_list.friend_photo, single_chat.sc_id, 
            single_chat.contact_id, single_chat.notification, single_chat.last_message,
            single_chat_message.sender_id, COUNT(CASE WHEN single_chat_message.status = 'unread' THEN 1 ELSE null END) AS unread_messages
            FROM contacts_list LEFT JOIN single_chat ON single_chat.user_id = ${req.query.id} AND single_chat.user_id = contacts_list.user_id 
            AND single_chat.contact_id = contacts_list.contact_id LEFT JOIN single_chat_message 
            ON single_chat_message.sc_id = single_chat.sc_id AND single_chat_message.sender_id != ${req.query.id}
            WHERE single_chat.contact_id is not null 
            GROUP BY contacts_list.contact_name, contacts_list.friend_photo, single_chat.sc_id, 
            single_chat.contact_id, single_chat.notification, single_chat.last_message,
            single_chat_message.sender_id
            ORDER BY last_time DESC`, (err, result) => {
