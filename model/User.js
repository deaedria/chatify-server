                        ...
                        bcrypt.genSalt(10, function (saltError, salt) {
                            bcrypt.hash(password, salt, function (hashingError, hash) {
                                if (!hashingError) {
                                    ...
                                    db.query('INSERT INTO users(password, email, phone, name) VALUES($1,$2,$3,$4)', [hash, email, phone, name], (err, response) => {
                                        db.query(`INSERT INTO contacts(contact_id, name, phone, username, photo_profile) SELECT t1.user_id, t1.name, t1.phone, t1.username, t1.photo_profile FROM users t1 LEFT JOIN contacts ON contacts.contact_id = t1.user_id WHERE contacts.contact_id IS NULL RETURNING *`, (error1, result1) => {
                                            if (!error1) {
                                                resolve({ message: 'user has been created', status: 201, data: newBody })
                                            } else {
                                                reject({ message: `create data failed ${error1}`, status: 500, data: {} })
                                            }
                                        })
                                    })
                                } else {
